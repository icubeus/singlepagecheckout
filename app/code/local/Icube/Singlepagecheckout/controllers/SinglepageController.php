...
class Icube_Singlepagecheckout_SinglepageController extends Mage_Checkout_OnepageController
{
    /**
    ...
    protected function _canShowForUnregisteredUsers()
    {
        $guestAllowed = Mage::getSingleton('customer/session')->isLoggedIn()
            || Mage::helper('checkout')->isAllowedGuestCheckout($this->getSinglepage()->getQuote())
    ...
    {
        if (!$this->getSinglepage()->getQuote()->hasItems()
            || $this->getSinglepage()->getQuote()->getHasError()
            || $this->getSinglepage()->getQuote()->getIsMultiShipping()) {
     ...
     *
     * @return Icube_Singlepagecheckout_Model_Type_Singlepage
     */
    public function getSinglepage()
    {
        return Mage::getSingleton('singlepagecheckout/type_singlepage');
    }
    ...
    protected function saveAddressData($data, $addressId, $type, $response = true)
    {
        ...
        $result = $this->getSinglepage()->$method($data, $addressId);
    ...
    protected function _getShippingMethodsHtml()
    {
        ...
        $layout->getUpdate()
            ->addHandle('checkout_singlestep_shippingmethod')
            ->merge('checkout_singlestep_shippingmethod');
        $layout->generateXml();
    ...
    protected function _getPaymentMethodsHtml()
    {
        $layout = Mage::getModel('core/layout');
        $layout->getUpdate()
            ->addHandle('checkout_singlestep_paymentmethod')
            ->merge('checkout_singlestep_paymentmethod');
        $layout->generateXml();
        $layout->generateBlocks();
        $output = $layout->getOutput();
        return $output;
    ...
    protected function _getReviewHtml()
    {
        $layout = Mage::getModel('core/layout');
        $layout->getUpdate()
            ->addHandle('checkout_singlestep_review')
            ->merge('checkout_singlestep_review');
        $layout->generateXml()
            ->generateBlocks();
    ...
    {
        $result = $this->getSinglepage()->saveShippingMethod($shippingMethod);
        ...
        $this->getSinglepage()->getQuote()->getShippingAddress()->setCollectShippingRates(true);
    ...
    protected function savePayment($data, $response = true)
    {
        ...
        try {
            $result = $this->getSinglepage()->savePayment($data);

            // get section and redirect data
            $redirectUrl = $this->getSinglepage()->getQuote()->getPayment()->getCheckoutRedirectUrl();
            if ($redirectUrl) {
    ...
    {
        if (!Mage::helper('singlepagecheckout')->singlePageCheckoutEnabled()) {
            Mage::getSingleton('checkout/session')
                ->addError($this->__('One Step checkout is disabled.'));
            $this->_redirect('checkout/cart');
            return;
        }
        $quote = $this->getSinglepage()->getQuote();
        ...
        $this->getSinglepage()->initCheckout();
        $this->loadLayout();
    ...
    public function saveMethodAction()
    {
        ...
        if ($this->getRequest()->isPost()) {
            ...
            $result = $this->getSinglepage()->saveCheckoutMethod($method);
            $this->getResponse()->setBody(Mage::helper('core')->jsonEncode($result));
        }
    }
    ...
    public function updateShippingMethodsAction()
    {
        ...
        if ($post) {
            ...
            if ($this->getSinglepage()->getQuote()->isVirtual()) {
    ...
    public function saveShippingMethodAction()
    {
        ...
        if ($this->getRequest()->isPost()) {
            ...
            $result = $this->getSinglepage()->saveShippingMethod($data);
            /*
            ...
            */
            ...
            //update totals
            $this->getSinglepage()->getQuote()->getShippingAddress()->setCollectShippingRates(true);
            $this->getSinglepage()->getQuote()->setTotalsCollectedFlag(false)->collectTotals()->save();
    ...
    public function submitOrderAction() {
        ...
        if ($post) {
            try {
                ...
                $method = $this->getRequest()->getPost('checkout_method');
                if ($method) {
                    $results[] = $this->getSinglepage()->saveCheckoutMethod($method);
                }
                ...
                if ($data = $this->getRequest()->getPost('payment', false)) {
                    $this->getSinglepage()->getQuote()->getPayment()->importData($data);
                }
                ...
                $this->getSinglepage()->saveOrder();
                $result['success'] = 1;

                $redirectUrl = $this->getSinglepage()->getCheckout()->getRedirectUrl();
                ...
                Mage::logException($e);
                Mage::helper('checkout')->sendPaymentFailedEmail($this->getSinglepage()->getQuote(), $e->getMessage());
            ...
            $this->getSinglepage()->getQuote()->save();
